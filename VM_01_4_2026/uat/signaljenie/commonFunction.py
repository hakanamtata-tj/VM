...
def who_tried(user):
    ...
    if profile is None:
        print(f"❌ SignalJenei {user['user']}  Profile not found. Exiting.")
        logging.error(f"❌ SignalJenei {user['user']}  Profile not found. Exiting.")
        send_telegram_message_admin(f"❌ SignalJenei {user['user']}  Profile not found. Exiting.")
        return
    else:
        print(f"ℹ️ SignalJenei Profile: {profile} | Current Time: {current_time}")
        logging.info(f"SignalJenei {get_profile(user)} tried to execute program at | Current Time: {current_time}")
        send_telegram_message_admin(f"ℹ️ SignalJenei {get_profile(user)} tried to execute program at | Current Time: {current_time}\n")
...
def load_db_open_trades(config, key, user, tradeGenie_id):
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    ...
    # 3. Log and return (Skipping Broker Checks)
    print(f"✅ {key} | Loaded open position from DB: {position_data['OptionSymbol']}")
    logging.info(f"✅ {key} | Loaded open position from DB: {position_data['OptionSymbol']}")
